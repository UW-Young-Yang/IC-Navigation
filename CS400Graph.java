...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {      
        ...
        PriorityQueue<Path> queueOfPath = new PriorityQueue<>();
        List<Vertex> listOfVertex = new LinkedList<>();
        Vertex currentVertex = vertices.get(start);
        Vertex endVertex = vertices.get(end);
        Path path = new Path(currentVertex);  // start
        listOfVertex.add(currentVertex);
        
        while (!currentVertex.equals(endVertex)) {
            // add the possible path to the queue
            for (Edge e : currentVertex.edgesLeaving) {
                if (listOfVertex.contains(e.target)) continue;  // ignore existing vertices
                Path nextPath = new Path(path);
                nextPath.extend(e);
                queueOfPath.offer(nextPath);
            }
            if (queueOfPath.isEmpty()) throw new NoSuchElementException("No path from start to end can be found");
            // find the shortest path and remove path containing repeated vertices
            path = queueOfPath.poll();
            while (listOfVertex.contains(path.end)) {
                path = queueOfPath.poll();
            }
            currentVertex = path.end;
            listOfVertex.add(currentVertex);
        }
        return path;
    }
